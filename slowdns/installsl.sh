...
setup_dnstt() {
	...
	chmod +x *
	mv * /etc/slowdns
	wget -O /etc/systemd/system/client.service "${REPO}slowdns/client" >/dev/null l 2>&1
	wget -O /etc/systemd/system/server.service "${REPO}slowdns/server" >/dev/null l 2>&1
